...
public class MyFirstTika {
    ...
    public static String parseUsingAutoDetect(String filename, TikaConfig tikaConfig,
        ...
        TikaInputStream stream = TikaInputStream.get(new FileInputStream(new File(filename)));
    ...
    public static String parseUsingComponents(String filename, TikaConfig tikaConfig,
        ...
        InputStream stream = TikaInputStream.get(new FileInputStream(new File(filename)));
        System.out.println("The MIME type (based on MAGIC) is: ["
                + mimeRegistry.detect(stream, metadata) + "]");

        stream = TikaInputStream.get(new FileInputStream(new File(filename)));
        ...
        langDetector.setMixedLanguages(true);
        langDetector.setShortText(false);
        List<LanguageResult> langs = langDetector.detectAll(handler.toString());
        String res="";
        for (LanguageResult lang : langs) {
            res+=lang.getLanguage()+",";
        }
        System.out.println("The language of this content is: [" + res + "]");
        //lang = langDetector.detect(handler.toString().substring(0,10));
        //System.out.println("The language of this content is: ["
        //                + lang.getLanguage() + "]");        
        LanguageDetectorExample exampleDetector=new LanguageDetectorExample();
        System.out.println("The language of this content is: ["
                + exampleDetector.detectLanguage(handler.toString().substring(0,10)) + "]");
